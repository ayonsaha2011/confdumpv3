...
impl IWbemClassWrapper {
    ...
    pub fn get_property(&self, property_name: &str) -> Result<Variant, WMIError> {
        ...
        // print!("vt property_name== {} ",  &property_name);
        ...
        unsafe {
            ...
            let property_value = match Variant::from_variant(vt_prop) {
                Ok(val) => val,
                Err(e) => { println!(" -------- Error for property_name - {} -------- {:?}", &property_name, e); Variant::String("".to_string()) }
            };
